...
const file_info_t* Find(const char *path) {
...
int Stat(const char *path, struct stat *st, struct fuse_file_info *fi)
{
	if (0 == strcmp("/", path)) {
	...
	const file_info_t *target = Find(path);
	if (!target) {
		return -ENOENT;
	}
...
int Open(const char *filepath, struct fuse_file_info *fi)
{
	const file_info_t *target = Find(filepath);
	if (!target) {
		return -ENOENT;
	}
...
int Read(const char *filepath, char *out, size_t sz, off_t off, struct fuse_file_info *fi) {
	const file_info_t *target = Find(filepath);
	if (target->size - off < sz - off) {
		sz = target->size - off;
	}
	if (sz > 0) {
